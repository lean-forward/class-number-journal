...
\institute{T. Baanen \at
           ...
           S. R. Dahmen \at
            ...
            A. Narayanan \at
                ...
                \email{a.narayanan20@imperial.ac.uk}
            \and
            F. A. E. Nuccio \at
                Univ Lyon, Université Jean Monnet Saint-Étienne, CNRS UMR 5208, Institut Camille Jordan, F-42023 Saint-\'Etienne, France \\
...
This paper is an extended version of a paper publishes in the ITP 2021 conference proceedings~\cite{ClassGroupsITP2021}. The additions to this paper, apart from several small local changes throughout the text, mainly concern the following.
%TODO Rewrite/clean up list below.
A (sub)section on the use of typeclasses; see Section~\ref{sec:typeclasses}.
The evolution of fraction rings in \mathlib at the end of Section~\ref{subsection : fields of fractions}.
A brief description of class groups for general domains, instead of only considering these for Dedekind domains; see the ends of Section~\ref{sec math background} and Section~\ref{subsec:class_group}.
Elaboration on unique factorization of ideals in Dedekind domains in Section~\ref{subsec:unique_ideal_factorization}.
More details on the finiteness of the class number in Section~\ref{sec:class-number}. %TODO
Elaboration on future directions in Section~\ref{sec:future_directions}. %TODO
Effective bounds on the class number? %TODO?
...
This implies that there is a $\Q$-algebra structure on $K$ (found by typeclass instance search), endowing $K$ with the $\Q$-vector space structure used in the hypothesis \mbox{\lean{[to\_finite\_dimensional : finite\_dimensional ℚ K]}}.
...
Indeed, the \mathlib definition of the rational numbers $\Q$ is a product type, not a quotient type, so it would not be considered the field of fractions of $\Z$ in this setup.
...
that this provided no easy way to view e.g. $\Q$ as the field of fractions of $\Z$,
...
This made it possible to view $\Q$ as the fraction field of $\Z$, by providing a suitable map \lean{int.fraction\_map : fraction\_map ℤ ℚ}.
...
Again we encountered a distinction between $\Q$ ``itself'' and $\Frac(\Z) = \lean{codomain int.fraction\_map}$,
...
Separating out these parameters finally allowed us to painlessly view $\Q$ as the fraction ring of $\Z$ while preserving the original $\Z$-algebra structure on $\Q$.
...
As briefly indicated before, we also formalized a proof that in a Dedekind domain every nonzero ideal can be expressed as a product of prime ideals in a unique way up to the order of the factors. It is actually known that for an integral domain this ideal factorization property is again equivalent to being a Dedekind domain, even when the uniqueness is dropped (which then follows as a property). We did not (yet) formalize these equivalent definitions, but instead formalized the unique ideal factorization property of a Dedekind domain \lean{D} by instantiating a unique factorization monoid structure on its ideals:

\begin{lstlisting}
instance ideal.unique_factorization_monoid :
  unique_factorization_monoid (ideal D) := -- proof omitted
\end{lstlisting}
For a brief general description of unique factorization monoids we refer the reader to the next section.
%TODO? perhaps move the (brief) description of unique factorization monoids from the next section to this point.
With much of the necessary definitions and properties already formalized, the formalization of the proof of this unique factorization result has been done in well under 100 lines of Lean code. One of the main mathematical ingredients (interesting in its own right) is that for ideals in a Dedekind domain, to contain is to divide:

\begin{lstlisting}
lemma ideal.dvd_iff_le {I J : ideal D} : (I ∣ J) ↔ J ≤ I := -- proof omitted
\end{lstlisting}

%TODO Anne? Perhaps say a little bit more about the proof (and/or how "smooth" is formalized)
...
Given a Dedekind domain $D$ with fraction field $K$, we formalized the notion of class group in Lean by defining a map \lean{to\_principal\_ideal D $:$ units K → units (fractional\_ideal D K)},
and defined the class group as
\begin{lstlisting}
def class_group :=
quotient_group.quotient (range to_principal_ideal)
\end{lstlisting}

Recall from Section~\ref{subsection:frac_ideals} that in the general case of an integral domain $D$ (which need not necessarily be Dedekind) the type of fractional ideals of $D$ is endowed with the structure of a commutative semiring. 
%TODO Anne?: perhaps add here (or in Section 4.2) something like \lean{instance comm\_semiring : comm\_semiring (fractional\_ideal S P) 
So also in this more general setting the group of invertible fractional ideals is well defined as \lean{units (fractional\_ideal D K)} and the quotient of this abelian group by the canonical image of the units of the (given) fraction field of $D$ gives rise to an abelian group \lean{class\_group} exactly as in the definition above.
%TODO Anne?: Elaborate/rewrite a bit? Put D in the main def?
...
We formalized, in the theorem \lean{class\_group.finite\_\-of\-\_admissible}, that the integral closure of $D$ in $K$ has a finite class group if $D$ has an ``admissible'' absolute value \lean{abs}.
...
The absolute value extends to a norm \lean{abs\_norm K abs $:$ integral\_closure D K → ℤ}.
We used the admissibility of \lean{abs} to find a finite set \lean{finset\_approx K L abs} of elements of $D$,
...
theorem exists_mem_finset_approx' (a b : integral_closure D L) :=
  ∃ (q : integral_closure D L) (r ∈ finset_approx K L abs),
  abs_norm K abs (r • a - q * b) < abs_norm K abs b
\end{lstlisting}
...
noncomputable instance : fintype
  (class_group (number_field.ring_of_integers K)) :=
class_group.finite_of_admissible K int.admissible_abs
\end{lstlisting} % Try to get the page break here.
\begin{lstlisting}
noncomputable instance [is_separable F K] : fintype
  (class_group (function_field.ring_of_integers $\Fq$ F K)) :=
class_group.finite_of_admissible F polynomial.admissible_card_pow_degree
\end{lstlisting}
