...
we show that each class in $\Cl{K}$ contains an ideal $J$ with $M \in J$,
where $M$ is the product of all elements of \lean{finset\_approx}.
Since the ideals of the Dedekind domain $S$ have unique factorization,
the ideal $\langle M \rangle$ spanned by $M$ has only finitely many divisors.
...
Restricting to quadratic fields, we see that the rank is only positive (namely 1) if the field is of the shape $\Q(\sqrt{d})$ for some positive integer $d$ (not a square).
...
given nonzero $D \in \Z$. It would be interesting to deal with some values of $D$ where no elementary techniques are available and factorization in the ring of integers of $\Q(\sqrt{D})$, along with class number info, wil resolve the equation.
