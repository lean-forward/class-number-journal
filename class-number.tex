...
The direct consequence of admissiblity of $x \mapsto \lvert x \rvert $, applied in practice,
is that for all $\epsilon \in \R_{>0},\ b \in D-\{0\},\ n \in \N$, and finite subsets $A \in D^n$ of cardinality $\operatorname{card}(\epsilon)^n+1$, there exist distinct $x,y \in D^n$ such that for all $i=1, \ldots, n$ we have $\lvert x_i \operatorname{mod} b - y_i \operatorname{mod}  b \rvert <\epsilon  \lvert b \rvert$.
