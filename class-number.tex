...
In fact, the previously mentioned examples of rings of integers are UFDs, but this is certainly not true for all rings of integers. For example, unique factorization \emph{does not} hold in $\mathcal{O}_{\Q(\sqrt{-5})}=\Z[\sqrt{-5}]$
...
In fact, for an integral domain, each nonzero ideal is a product of prime ideals if and only if all nonzero fractional ideals are invertible; the uniqueness follows separately (see \cite[Proposition 3.8]{Neukirch} and \cite[Proposition 12.2.8]{Ireland-Rosen} for either implication).
