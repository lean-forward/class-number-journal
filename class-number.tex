...
where $d \in \Z$ is not a square. Similarly, adding a cubic root of some $d\in\Z$ which is not a cube leads to the number field $\QQ(\sqrt[3]{d})$: it has degree $3$ but not all cubic number fields arise in this way.
...
Compared to Haskell, Lean's typeclasses have few structural restrictions: notably, classes and instances can depend on any term, instances may overlap, classes can apply to multiple types and can have functional dependencies.
% TODO: cite my upcoming typeclass paper?
...
determining whether information about an object should be carried in the object itself (bundled), or passed as a separate value (unbundled).
...
Similarly, the formalization of admissible absolute values discussed in Section~\ref{subsec:finiteness} features a bundled structure \lean{absolute\_value} which includes a map along with proofs stating this map is an absolute value,
...
In contrast, bundling hypotheses means each subset of hypotheses requires its own structure declaration;
...
Thus, ensuring definitional equality for instances means overlapping instances will not lead to conflicts when one instance is expected and another is found.
...
\end{lstlisting}
From the existence of $T$ and $T'$ we concluded $I$ is indeed finitely generated, finishing the proof.
...
Examples in \mathlib of UFMs are the unique factorization domains $\N$ and $\Z$, and for any UFM \lean{α}, the quotient of \lean{α} by the subgroup of invertible elements \lean{associates α}.
...
Finally, we mention that since our project became available in the \mathlib library, a team led by Brasca has begun formalizing Fermat's Last Theorem for regular primes. Fermat's Last Theorem is the assertion that, for all~$n\geq 3$,
\begin{equation}\tag*{\refFLT{n}}
\forall x,y,z \in \Z, \quad x^n+y^n=z^n\Longrightarrow x\cdot y \cdot z =0
\end{equation}
In other words, any integral solution to the Fermat equation $X^n+Y^n=Z^n$ must be trivial. It is immediate to see that, for every~$n\geq 3$, \refFLT{n} is equivalent to the validity of \refFLT{p} for all prime numbers~$p$ dividing~$n$, and therefore the only interesting exponents to consider are prime numbers. Now, recall that a prime number $p$ is said to be \emph{regular} if it does not divide the order of the class group $\Cl_{\QQ(\zeta_p)}$ of the number field obtained by adjoining to~$\QQ$ a primitive $p$th root of unity; equivalently,~$\zeta_p^p=1$ and~$\zeta_p\neq 1$ or, again equivalently,~$\zeta_p$ is a root of the irreducible polynomial
\[
\frac{X^p-1}{X-1}=X^{p-1}+X^{p-2}+\cdots +X+1.
\]
A classical result, due to Kummer's work in 1847, is that \refFLT{p} is true for every \emph{regular} prime number~$p$. This is the result Brasca and his team have been formalizing in Lean 3 in the on-going work~\cite{Bra21}, and it evidently requires the finiteness of the class group in order to define the notion of a \emph{regular prime}\footnote{It is actually possible to simply \emph{define} a regular prime without mentioning class groups, in terms of divisibility of some Bernoulli number, but this definition would at any rate need to be translated in terms of class numbers in order to implement Kummer's proof.}. Moreover, most arguments occurring in Kummer's proof pertain to the structure of the ring of integers $\Z[\zeta_p]=\OK[\QQ(\zeta_p)]$ as a Dedekind domain, and our work lies at the core of the formalization of these structures.
