...
This implies that there is a $\Q$-algebra structure on $K$ (found by typeclass instance search), endowing $K$ with the $\Q$-vector space structure used in the hypothesis \mbox{\lean{[fd : finite\_dimensional ℚ K]}}.

Similarly, we defined the function field $K$ over a finite field $\Fq$ using the following typeclass:
\begin{lstlisting}
class is_function_field_over ($\Fq$ F K : Type*)
  [field $\Fq$] [fintype $\Fq$] [field F] [field K]
  [algebra (polynomial $\Fq$) F] [algebra F K]
  [is_fraction_ring (polynomial $\Fq$) F] : Prop :=
[fd : finite_dimensional F K]
\end{lstlisting}
% Curly brackets mark implicit parameters inferred through unification, such as \lean{\{$\Fq$ F : Type*\}}.
The typeclass parameter \lean{[is\_fraction\_ring (polynomial $\Fq$) F]} witnesses that $F$ is a fraction field of the polynomial ring $\Fq[q][t]$.
We present a more detailed analysis of \lean{is\_fraction\_ring} in Section \ref{subsection : fields of fractions}.
...
lemma fractional_mul (I J : fractional_ideal R F) :
  is_fractional f (I * J : submodule R F) := _ -- proof omitted
...
\begin{lstlisting}
lemma fractional_ideal.is_unit [is_dedekind_domain D]
  (I : fractional_ideal R F) (hne : I ≠ ⊥) : is_unit I
\end{lstlisting}
...
The first step was to show that $K$ is a field of fractions for the integral closure, namely, there is an instance \lean{is\_fraction\_ring\_of\_finite\_extension R F K : is\_fraction\_ring S K}.
