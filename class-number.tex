...
In other words, each element has a unique type. The natural number 0 has type \lean{ℕ}, and the rational 0 has type \lean{ℚ}. One can then idenntify \lean{0 : ℕ} with \lean{0 : ℚ} using a map \lean{ℕ → ℚ} called a coercion (generally denoted by an arrow); 
...
For now, we point out that there are many fields $K$ that are isomorphic to the field of rational functions $\Fq[q](t)$; we provided a theorem \lean{function\_field\_iff} that shows the choice of $K$ does not matter.
...
where \lean{is\_domain} is a typeclass asserting the ring is nontrivial and there are no zero divisors,
